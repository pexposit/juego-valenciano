...
- `backend/`: verifica el JWT con Supabase, obtiene contexto, llama a OpenRouter con salida JSON Schema, persiste progreso y aplica XP.
...
Despliega `frontend` en Vercel con sus `VITE_*`. El backend incluye `Dockerfile` y puede desplegarse en Railway, Render o Fly.io; configura `FRONTEND_ORIGIN`, secretos de Supabase y `OR_API_KEY`. Por defecto usa `minimax/minimax-m3:free` y dispone de modelos gratuitos de respaldo; puedes cambiarlo con `OPENROUTER_MODEL`. Nunca expongas `SUPABASE_SERVICE_ROLE_KEY` ni claves de proveedores en el frontend.
